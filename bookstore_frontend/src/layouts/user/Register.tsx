      ...
      try {
        ...
        if (response.ok) {
          ...
          if (data && data.error) {
            ...
            if (data.message === 'IdInvalidException') {
              setGeneralError('Không thể đăng ký. Vui lòng kiểm tra lại thông tin.');
            }
            // Kiểm tra các lỗi cụ thể
            else if (data.message.includes('username') || data.message.includes('tên đăng nhập') || data.message.includes('tenDangNhap')) {
              setUserNameError('Tên đăng nhập đã tồn tại');
            }
            else if (data.message.includes('email')) {
              setEmailError(data.message || 'Email này đã được đăng ký');
            }
